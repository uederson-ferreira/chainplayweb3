  ...
  const createRound = useCallback(
    async (params: RoundCreationParams) => {
      ...
      try {
        // ✅ VALIDAÇÕES
        if (
          params.numeroMaximo < GAME_CONSTANTS.MIN_NUMERO_MAXIMO ||
          params.numeroMaximo > GAME_CONSTANTS.MAX_NUMERO_MAXIMO
        ) {
          throw new Error(
            `Número máximo deve estar entre ${GAME_CONSTANTS.MIN_NUMERO_MAXIMO} e ${GAME_CONSTANTS.MAX_NUMERO_MAXIMO}`
          );
        }

        const taxaFloat = parseFloat(params.taxaEntrada);
        if (
          taxaFloat < parseFloat(GAME_CONSTANTS.MIN_TAXA_ENTRADA) ||
          taxaFloat > parseFloat(GAME_CONSTANTS.MAX_TAXA_ENTRADA)
        ) {
          throw new Error(
            `Taxa deve estar entre ${GAME_CONSTANTS.MIN_TAXA_ENTRADA} e ${GAME_CONSTANTS.MAX_TAXA_ENTRADA} ETH`
          );
        }

        if (
          params.timeoutHoras < GAME_CONSTANTS.MIN_TIMEOUT_HORAS ||
          params.timeoutHoras > GAME_CONSTANTS.MAX_TIMEOUT_HORAS
        ) {
          throw new Error(
            `Timeout deve estar entre ${GAME_CONSTANTS.MIN_TIMEOUT_HORAS} e ${GAME_CONSTANTS.MAX_TIMEOUT_HORAS} horas`
          );
        }
        ...
        if (
          ...
          error.message?.includes("Número máximo") ||
