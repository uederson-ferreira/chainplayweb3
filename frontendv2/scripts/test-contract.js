...
async function testContract() {
  ...
  try {
    ...
    if (cartelaCode) {
      console.log('✅ Contrato Cartela deployado (tamanho:', cartelaCode.length, ')')
    } else {
    ...
    if (bingoCode) {
      console.log('✅ Contrato Bingo deployado (tamanho:', bingoCode.length, ')')
    } else {
    ...
    for (const testAddress of testAddresses) {
      try {
        ...
        console.log(`🔑 ${testAddress}: ${isOperator ? 'É OPERADOR' : 'não é operador'}`)
