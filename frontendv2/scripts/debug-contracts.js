...
async function debugContracts() {
  ...
  try {
    ...
    const cartelaCode = await publicClient.getBytecode({ address: CARTELA_ADDRESS })
    console.log(`📋 Cartela (${CARTELA_ADDRESS}):`)
    console.log(`   Código: ${cartelaCode ? `${cartelaCode.slice(0, 20)}... (${cartelaCode.length} chars)` : "❌ SEM CÓDIGO"}`)

    const bingoCode = await publicClient.getBytecode({ address: BINGO_ADDRESS })
    console.log(`🎲 Bingo (${BINGO_ADDRESS}):`)
    console.log(`   Código: ${bingoCode ? `${bingoCode.slice(0, 20)}... (${bingoCode.length} chars)` : "❌ SEM CÓDIGO"}`)
        ...
        const cartela0 = await publicClient.readContract({
          ...
          abi: [{
            ...
            "outputs": [
              ...
              {"name": "numerosRegistrados", "type": "bool", "internalType": "bool"},
    ...
    if (cartelaCode) {
      try {
        ...
        if (logs.length > 0) {
          console.log(`   Primeiro evento: Bloco ${logs[0].blockNumber}, Topics: ${logs[0].topics.length}`)
        }
      } catch (error) {
    ...
    for (const account of accounts) {
      try {
        ...
        console.log(`❌ Erro ao verificar saldo de ${account}: ${error.message}`)
      }
    }
