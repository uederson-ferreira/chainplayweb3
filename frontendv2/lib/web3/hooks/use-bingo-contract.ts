// lib/web3/hooks/use-bingo-contract-FIXED.ts
"use client";
...
export function useBingoContract() {
  ...
  // ========================================
  // LOGS DE DEBUG E VALIDAÇÃO (CORRIGIDO)
  // ========================================
  console.log("🔍 Debug do contrato Bingo:", {
    ...
    temIniciarRodada: BINGO_ABI.filter((item) => item.type === "function").find(
      (f) => f.name === "iniciarRodada"
    ),
    temOperadores: BINGO_ABI.filter((item) => item.type === "function").find(
      (f) => f.name === "operadores"
    ),
    // ✅ REMOVIDO: getTotalRodadas não existe no contrato
  });

  // ========================================
  // FUNÇÃO iniciarRodada MELHORADA (CORRIGIDO)
  // ========================================
  const iniciarRodada = async (
    ...
    const iniciarRodadaABI = funcoesBingo.find(
      (f) => f.name === "iniciarRodada"
    );
    ...
    if (!Array.isArray(padroesVitoria) || padroesVitoria.length !== 4) {
      throw new Error(
        "❌ padroesVitoria deve ser array com exatamente 4 elementos"
      );
    }
    ...
    // ========================================
    // CHAMADA writeContract CORRIGIDA
    // ========================================
    try {
      ...
      if (
        ...
        throw new Error(
          "Transação rejeitada pelo contrato - verifique se você é operador"
        );
      } else if (error?.message?.includes("network")) {
        throw new Error("Erro de rede - verifique sua conexão");
      } else {
        throw new Error(
          `Erro na transação: ${error?.message || "Erro desconhecido"}`
        );
      }
    }
  };
  ...
  const participar = async (
    ...
    console.log("🎯 Participando da rodada:", {
      rodadaId,
      cartelaId,
      taxaEntrada,
    });
...
// ✅ REMOVIDO: useTotalRodadas() porque getTotalRodadas não existe no contrato
// Em vez disso, usar a estratégia no use-enhanced-active-rounds-FIXED.ts
...
export function useNumerosSorteados(rodadaId?: bigint) {
  ...
  return {
    numerosSorteados: numerosSorteados
      ? numerosSorteados.map((n) => Number(n))
      : [],
  };
}
